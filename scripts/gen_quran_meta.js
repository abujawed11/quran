...
for (let page = 1; page <= TOTAL_PAGES; page++) {
  ...
  // New format: { page, image, ayahs: { "S:A": [...] } }
  // Old format: { "S:A:wordIndex": {x,y,w,h}, ... }
  const ayahKeys = data.ayahs
    ? Object.keys(data.ayahs)                          // new format
    : Object.keys(data).filter((k) => /^\d+:\d+:/.test(k)).map((k) => k.slice(0, k.lastIndexOf(":"))); // old format

  const uniqueKeys = [...new Set(ayahKeys)];

  for (const ayahKey of uniqueKeys) {
...
// Build AYAH_PAGES as a compact JS object literal
const ayahPagesEntries = Object.entries(ayahToPage)
  .sort(([a], [b]) => {
...
/** Maps "surah:ayah" → page number. Used for direct seek navigation. */
export const AYAH_PAGES = {
${ayahPagesEntries}
};
